...
class NormalTanhDistribution(ParametricDistribution):
  ...
  def __init__(self, event_size, min_std=0.01):
...
class Beta:
  """Beta distribution, by qxy"""

  def __init__(self, aplah, beta):
    self.aplah = aplah + jnp.ones(aplah.shape)
    self.beta = beta + jnp.ones(beta.shape)
    self.aplah_ = aplah
    self.beta_ = beta

  def sample(self, seed):
    return jax.random.beta(seed, a= self.aplah, b= self.beta, shape=self.aplah.shape) 

  def mode(self):
    return(self.aplah_/(self.aplah_ + self.beta_))

  def log_prob(self, x):
    log_beta = jax.scipy.special.betaln(self.aplah,self.beta)
    return -log_beta + self.aplah_ * jnp.log(x) + self.beta_ * jnp.log(jnp.ones(self.beta.shape) - x)

  def entropy(self):
    log_beta = jax.scipy.special.betaln(self.aplah,self.beta)
    entropy = log_beta - self.aplah_ * jax.scipy.special.digamma(self.aplah) - self.beta_ * jax.scipy.special.digamma(self.beta) + (self.aplah_ + self.beta_) * jax.scipy.special.digamma(self.aplah + self.beta)
    return entropy * jnp.ones_like(self.aplah)


class LinearBijector:
  def forward(self, x):
    return 2. *(x - 0.5 *jnp.ones(x.shape))

  def inverse(self, y):
    return y / 2 + 0.5*jnp.ones(y.shape)
...
class BetaDistribution(ParametricDistribution):
  ...
  def __init__(self, event_size):
     super().__init__(
        param_size=2 * event_size,
        postprocessor=LinearBijector,
        event_ndims=1,
        reparametrizable=True)
    

  def create_dist(self, parameters):
    aplah, beta = jnp.split(parameters, 2, axis=-1)
    aplah = jax.nn.softplus(aplah)
    beta = jax.nn.softplus(beta) 
    return Beta(aplah=aplah, beta=beta)
